...
class Application(Tk):
    ...
    #Figures to Canvas
        ...
        self.plotTR.set_ylabel("Current (A)")
        self.plotTR.set_yscale('log')
        self.plotTR.grid(True)
        self.plotTR.step(self.t,self.TR)
        self.plotTR.step(self.t,TR)
    ...
    def actualizeFigure(self, TL=[0], TR=[0], BL=[0], BR=[0]):
    #Function to generate values to plot
        if self.state == "SINGLE":
            self.plotSingleSequence()
            self.setFigures(TL, TR, BL, BR)
    ...
    #Function to generate preview of the sequence as it should be performed by the instrument
        if self.liste2.current()==1 and self.liste3.current()==0:
            ...
            self.BL = 1e6 - 1e6*self.t/self.t[-1]
            self.TR = self.TL*0       
            ...
            self.BL = 10 + 1e6*self.t/self.t[-1]
            self.TR = self.TL*0       
            self.BR = self.TL*self.TR
    ...
    #Function to generate the waveform to send to the instrument for a Single sequence
        ...
        if R >= 1e6 and self.liste2.current()==0:            
            ...
            if self.liste3.current()==0 and answer==True:
                ...
                [Um, Im] = generateSingleCurrentWaveform(instr, self.TR, lim)
            
            self.actualizeFigure(TL=Um, TR=Im, BL=Um/Im, BR=Um*Im)
    ...
    #Function to generate the waveform to send to the instrument for a Cycling sequence
        ...
        if self.liste3.current()==0:
            [iterations, R, Um, Im] = generateCyclingVoltageWaveform(instr, s1, lim1, s2, lim2)

        elif self.liste3.current()==1: 
            [iterations, R, Um, Im] = generateCyclingCurrentWaveform(instr, s1, lim1, s2, lim2)

        self.actualizeFigure(TL=Um, TR=Im, BL=Um, BR=Im)
